...
def gen_discrete_map(im_height, im_width, points):
    ...
    p_index = torch.from_numpy(p_h * im_width + p_w).long()  # Convert to torch.int64
    discrete_map = torch.zeros(im_width * im_height).scatter_add_(0, index=p_index, src=torch.ones(num_gt)).view(im_height, im_width).numpy()
